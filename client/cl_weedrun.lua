...
RegisterNetEvent('ps-weedplanting:client:StartPackage', function(data)
    ...
    -- end
    ----------------------- edited parts -------------------------------
    local hasAnyPackedWeedItem = false
    for itemName, itemData in pairs(Shared.PackedWeedItems) do
        local hasItem = QBCore.Functions.HasItem(itemName, 20) --1
        if hasItem then
            hasAnyPackedWeedItem = itemName -- true
            break
        end
    end
    if not hasAnyPackedWeedItem then
        QBCore.Functions.Notify(_U('dont_have_anything'), 'error', 2500)
        return
    end
    ----------------------- edited parts -------------------------------
