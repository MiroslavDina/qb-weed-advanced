...
function findBranchByType(type)
    for itemName, itemData in pairs(Shared.BranchItems) do
...
function findSeedByType(type)
    for itemName, itemData in pairs(Shared.SeedItems) do
        if itemData.type == type then
            return itemName
        end
    end
...
RegisterNetEvent('ps-weedplanting:server:HarvestPlant', function(netId)
    ...
    if DoesEntityExist(entity) then
        ...
        if WeedPlants[entity].gender == 'female' then
            ...
            local seed = findSeedByType(WeedPlants[entity].type)
            local seedItemData = Shared.SeedItems[seed]
            if not seedItemData then
                return
            end

            Player.Functions.AddItem(seedItemData.rewardItem, 1, false, info)
            TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[seedItemData.rewardItem], 'add', 1)
...
RegisterNetEvent('ps-weedplanting:server:CreateNewPlant', function(coords, weedType)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    local seedByType = findSeedByType(weedType)
    if not seedByType then
        print(string.format("Cannot find seed item configuration for type %s", weedType))
        return
    end

    if Player.Functions.RemoveItem(seedByType, 1) and Player.Functions.RemoveItem(Shared.PlantTubItem, 1) then
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[seedByType], 'remove', 1)
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[Shared.PlantTubItem], 'remove', 1)
        local ModelHash = Shared.WeedProps[1]
        local plant = CreateObjectNoOffset(ModelHash, coords.x, coords.y, coords.z + Shared.ObjectZOffset, true, true, false)
        FreezeEntityPosition(plant, true)
        local time = os.time()
        MySQL.insert('INSERT into weedplants (coords, time, fertilizer, water, gender, type) VALUES (:coords, :time, :fertilizer, :water, :gender, :type)', {
...
for itemName, itemData in pairs(Shared.SeedItems) do
    QBCore.Functions.CreateUseableItem(itemName, function(source)
        ...
        if tub ~= nil then
            TriggerClientEvent("ps-weedplanting:client:UseWeedSeed", source, itemData.type)
        else
