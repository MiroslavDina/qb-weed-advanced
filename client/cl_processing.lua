...
RegisterNetEvent('ps-weedplanting:client:ProcessBranch', function()
    local hasAnyBranchItem = false
    for itemName, itemData in pairs(Shared.WeedBranchItems) do
        local hasItem = QBCore.Functions.HasItem(itemName, 1)
        if hasItem then
            hasAnyBranchItem = itemName -- true
            break
        end
    end

    if not hasAnyBranchItem then
    ...
    local hasScissors = false
    local hasBag = false

    for itemName, itemData in pairs(Shared.AccessoriesItems) do
        local hasItem = QBCore.Functions.HasItem(itemName, 1) -- Check if the player has 1 of the specified item type

        if itemName == Shared.GearItems.TRIMMING_SCISSORS and hasItem then
            hasScissors = true
        elseif itemName == Shared.GearItems.EMPTY_WEED_BAG and hasItem then
            hasBag = true
        end
       if hasScissors and hasBag then
            break  
        end
    end

    local hasScissors = QBCore.Functions.HasItem(Shared.GearItems.TRIMMING_SCISSORS, 1)
    local hasBag = QBCore.Functions.HasItem(Shared.GearItems.EMPTY_WEED_BAG, 20)

    if not hasScissors then 
        QBCore.Functions.Notify(_U('dont_have_scissors'), 'error', 2500)
        return
    end
    
    if not hasBag and hasScissors then
        QBCore.Functions.Notify(_U('dont_have_bags'), 'error', 2500)
        return
    end
...
RegisterNetEvent('ps-weedplanting:client:PackDryWeed', function()
    local hasAnyWeedItem = false
    for itemName, itemData in pairs(Shared.WeedItems) do
        local hasItem = QBCore.Functions.HasItem(itemName, 1)
        if hasItem then
            hasAnyWeedItem = itemName --true
            break
        end
    end

    if not hasAnyWeedItem then
        QBCore.Functions.Notify(_U('not_enough_dryweed'), 'error', 2500)
        return
    end
     -------------- edited parts --------------------
    local hasScissors = false
    ...
    for itemName, itemData in pairs(Shared.AccessoriesItems) do
        ...
        if hasScissors and hasBag then
            break  -- Exit the loop early once both items are found
        end
    end
    ...
    if not hasBag and hasScissors then
        QBCore.Functions.Notify(_U('dont_have_bags_pack'), 'error', 2500)
        return
    end
    -----------------------------------------------------------
    local ped = PlayerPedId()
