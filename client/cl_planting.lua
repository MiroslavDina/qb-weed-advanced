...
RegisterNetEvent('ps-weedplanting:client:UseWeedSeed', function(weedType)
    if not weedType then return end
        ...
        if hit == 1 then
            ...
            if IsControlJustPressed(0, 38) then
                ...
                QBCore.Functions.Progressbar('spawn_plant', _U('place_sapling'), 2000, false, true, {
                    ...
                    TriggerServerEvent('ps-weedplanting:server:CreateNewPlant', dest, weedType)
                    planted = false
                    seedUsed = false
                    ClearPedTasks(ped)
        ...
        elseif result.growth == 100 then -- Harvest
            if PlayerJob.type == Shared.CopJob and PlayerJob.onduty then
                exports['qb-menu']:openMenu({
                    ...
                    {
                        header = 'Druh: ' .. result.type,------------------------------------------------------------------pridane
                        icon = 'fas fa-mars',
                        params = {
                            args = data.entity,
                        }
        ...
        elseif result.gender == 'female' then -- Option to add male seed
            if PlayerJob.type == Shared.CopJob and PlayerJob.onduty then
                exports['qb-menu']:openMenu({
                    ...
                    {
                        header = 'Rast: ' .. result.growth .. '%' .. ' - Úroveň: ' .. result.stage,
                ...
                exports['qb-menu']:openMenu({
                    ...
                    {
                        header = 'Hnojivo: ' .. result.fertilizer .. '%',
